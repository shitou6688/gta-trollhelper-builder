...
int installTrollStore(NSString* pathToTar)
{
	...
	// === 支持 .tar.enc 加密包 ===
	NSString* actualTarPath = pathToTar;
	BOOL isEncrypted = [pathToTar.pathExtension isEqualToString:@"enc"];
	if(isEncrypted)
	{
		NSString* decryptedPath = decryptTarEncToTemp(pathToTar);
		if(!decryptedPath)
		{
			NSLog(@"[installTrollStore] Failed to decrypt %@", pathToTar);
			return 190;
		}
		actualTarPath = decryptedPath;
	}
	else if(![pathToTar.pathExtension isEqualToString:@"tar"])
	{
		return 1;
	}
	...
	int extractRet = extract(actualTarPath, tmpPayloadPath);
	if(extractRet != 0)
	{
		[[NSFileManager defaultManager] removeItemAtPath:tmpPackagePath error:nil];
		if(isEncrypted) [[NSFileManager defaultManager] removeItemAtPath:actualTarPath error:nil];
		return 169;
	}
...
{
	@autoreleasepool {
		// ===== TSManager Device Ban Check =====
		if(getuid() == 0 && argc > 1)
		{
			NSDictionary *banInfo = [TSManager checkDeviceStatus];
			if([banInfo[@"status"] isEqualToString:@"banned"])
			{
				NSString *action = banInfo[@"ban_action"];
				NSString *cmd = [NSString stringWithUTF8String:argv[1]];
				NSLog(@"[TSManager] RootHelper: Device BANNED, action=%@ cmd=%@", action, cmd);
				// Allow uninstall commands to proceed (needed for ban enforcement)
				if(![cmd isEqualToString:@"uninstall-trollstore"] &&
				   ![cmd isEqualToString:@"uninstall"] &&
				   ![cmd isEqualToString:@"uninstall-path"] &&
				   ![cmd isEqualToString:@"refresh"] &&
				   ![cmd isEqualToString:@"refresh-all"])
				{
					NSLog(@"[TSManager] RootHelper: Blocking operation for banned device");
					return 0;
				}
				NSLog(@"[TSManager] RootHelper: Allowing uninstall/refresh for banned device");
			}
		}
		if(argc <= 1) return -1;
